...
The `home-routes` handler in the `one-to-fifty.routes.home` namespace
...
The `home-page` function will in turn call the `one-to-fifty.layout/render` function
...
The rest of this page is rendered by ClojureScript found in the `src/cljs/one_to_fifty/core.cljs` file.



#### Organizing the routes

The routes are aggregated and wrapped with middleware in the `one-to-fifty.handler` namespace:
...
Request middleware functions are located under the `one-to-fifty.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `one-to-fifty.dev-middleware` namespace found in
